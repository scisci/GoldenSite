...
typedef std::unordered_map<unsigned int, std::vector<Complement> > ComplementsReverseMap;
...
public:
  ...
  inline const double& GetRatio(RatioIndex index) const
  {
    assert(index < ratios_.size());
    return ratios_[index];
  }
  
  inline double& operator[] (RatioIndex index)
  {
    return ratios_[index];
  }
  
  inline const double& operator[] (RatioIndex index) const
  {
    return ratios_[index];
  }
  ...
  RatioIndex FindClosestIndex(double ratio) const
  {
    ...
    if (it == ratios_.begin()) {
      ...
      return (RatioIndex)ratios_.size() - 1;
    } else {
      RatioIndex index = (RatioIndex)(it - ratios_.begin());
      return (ratio - *(it - 1) <= *it - ratio) ? index - 1 : index;
    }
  }
  
  RatioIndex FindClosestIndex(double ratio, const std::set<RatioIndex>& subset) const
  {
    // Returns greater than or equal to ratio
    std::set<int>::const_iterator it;
    
    double closest_distance = DBL_MAX;
    RatioIndex closest_index = kRatioIndexInvalid;
  ...
  RatioIndex FindClosestInverseIndex(double ratio) const
  {
    // Returns greater than or equal to ratio
    return FindClosestIndex(1 / ratio);
  }
  
  inline RatioIndex FindInverseRatioIndex(RatioIndex ratio_index) const
  {
    ...
    if (fabs(ratios_[closest_index] - inverse_ratio) < kRatioEpsilon) {
      return closest_index;
    }
    
    return kRatioIndexInvalid;
  }
      ...
      for (j = 0; j < ratio_count; ++j) {
        if (ratios_[j] < r) {
          for (k = j; k < ratio_count; ++k) {
            if (fabs(r - ratios_[j] - ratios_[k]) < kRatioEpsilon) {
              complements_[i].push_back(Split(kSplitTypeVertical, j, k));
              break;
            }
          }
        }
        ...
        if (ratios_[j] < r) {
          for (k = j; k < ratio_count; ++k) {
            if (fabs(r - ratios_[j] - ratios_[k]) < kRatioEpsilon) {
              ...
              complements_[i].push_back(Split(kSplitTypeHorizontal, invJ, invK));
              break;
            }
          }
        }
